...
package-linux:
    @mkdir -p dist
    ouch c --slow ./target/{{ linux }}/release/tasker-cli ./dist/tasker-cli-linux.tar.gz

# Package Windows executable
package-windows:
    @mkdir -p dist
    ouch c --slow ./target/{{ windows }}/release/tasker-cli.exe ./dist/tasker-cli-windows.zip

# Package Mac executables
package-mac:
    @mkdir -p dist
    ouch c --slow ./target/{{ mac_x86 }}/release/tasker-cli ./dist/tasker-cli-mac-x86.zip
    ouch c --slow ./target/{{ mac_arm }}/release/tasker-cli ./dist/tasker-cli-mac-arm.zip
